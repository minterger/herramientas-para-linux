  ...
  sockexp=$(grep "DEFAULT_HOST =" proxy.py | awk -F : '{print $2}' | awk -F "'" '{print $1}')
	...
	if [[ "$resposta" = '1' ]]; then
		if ps x | grep proxy.py|grep -v grep 1>/dev/null 2>/dev/null; then
		    ...
		    fun_inisocks () {
		    	sleep 1
		    	var_ptsks2=$(sed -n "12 p" proxy.py | awk -F = '{print $2}')
		    	sed -i "s/$var_ptsks2/ $porta/g" proxy.py
		    	sleep 1
		    	screen -dmS proxy python proxy.py
		    	if grep -w "SSHPlus" /etc/rc.local > /dev/null 2>&1; then
		    		echo ""
		    	else
		    		sed -i '$ iscreen -dmS proxy python proxy.py' /etc/rc.local
			    fi
		    }
		    echo ""
		...
		if ps x | grep proxy.py|grep -v grep 1>/dev/null 2>/dev/null; then
			...
			abrirptsks () {
				sleep 1
				screen -dmS proxy python proxy.py $porta
				sleep 1
			}
			fun_bar 'abrirptsks'
			echo ""
		...
		if ps x | grep proxy.py|grep -v grep 1>/dev/null 2>/dev/null; then
			clear
			msgsocks=$(cat proxy.py |grep -E "MSG =" | awk -F = '{print $2}' |cut -d "'" -f 2)
			echo -e "\E[44;1;37m             PROXY SOCKS              \E[0m"
			echo ""
			...
			fun_msgsocks () {
				msgsocks2=$(cat proxy.py |grep "MSG =" | awk -F = '{print $2}')
				sed -i "s/$msgsocks2/ '$msgg'/g" proxy.py
				sleep 1
			}
			echo ""
		...
		if ps x | grep proxy.py|grep -v grep 1>/dev/null 2>/dev/null; then
			clear
			msgsocks=$(cat proxy.py |grep -E "DEFAULT_HOST =" | awk -F = '{print $2}' |cut -d "'" -f 2| awk -F ":" '{print $2}')
			echo -e "\E[44;1;37m             PROXY SOCKS              \E[0m"
			echo ""
			...
			fun_msgsocks () {
				msgsocks2=$(cat proxy.py |grep "DEFAULT_HOST =" | awk -F = '{print $2}' | awk -F : '{print $2}')
				sed -i "s/$msgsocks2/ $msgg'/g" proxy.py
				sleep 1
			}
			echo ""
